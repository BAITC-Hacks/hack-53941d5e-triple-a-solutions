...
export function createWorkflows({ data: source, provider }) {
  async function developmentPlan(request) {
    ...
    const result = await provider.run({ name: 'development_plan', prompt: PLAN_PROMPT, schema,
    ...
    return { ...metadata, plan: value || fallback, message: result.source === 'ai' ? 'AI выбрал маршрут. Порядок шагов и прирост навыков проверены приложением.' : result.message };
  }
