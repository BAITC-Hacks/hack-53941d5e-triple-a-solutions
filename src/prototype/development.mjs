...
export function buildDevelopmentPaths(data, state, input = {}, rules = {}) {
  ...
  const catalog = data.events.filter(e => !e.mandatory && e.target_roles.includes(person.role) && e.target_grades.includes(person.grade)
    && (!new Set(state.done).has(e.event_id) || e.event_id === 'EV_036') && !state.simulated.includes(e.event_id)
  ...
  for (let depth = 0; depth < options.maxSteps; depth++) {
    ...
    for (const node of beam) {
      ...
      const candidates = model.available(person).filter(item => item.skillGains.some(s => s.level < (needed.get(s.id) || 0))).map(item => ({ ...item,
        start: rules.sessionPrograms?.[item.event.event_id]
          ? (() => { const first = [...item.event.upcoming_sessions].sort()[0]; const start = first < data.asOf ? data.asOf : first; return start >= node.date ? start : null; })()
          : item.event.format === 'self_paced' ? node.date : [...item.event.upcoming_sessions].sort().find(date => date >= node.date),
      ...
      for (const item of candidates.slice(0, 8)) {
        ...
        const estimate = addDays(item.start, Math.max(1, Math.ceil(item.event.duration_hours / options.weeklyHours * 7)));
        const lastSession = rules.sessionPrograms?.[item.event.event_id] ? [...item.event.upcoming_sessions].sort().at(-1) : null;
        const end = lastSession && lastSession > estimate ? lastSession : estimate;
...
export function planProgress(plan, state) {
  ...
  if (plan.baseDone) {
    const actual = new Set(state.done), baseline = new Set(plan.baseDone);
    const extra = [...actual].filter(id => !baseline.has(id));
    const done = plan.steps.findIndex(step => !actual.has(step.event_id));
    const count = done < 0 ? plan.steps.length : done;
    return { valid: extra.length === count && extra.every(id => plan.steps.slice(0, count).some(s => s.event_id === id)), done: count };
  }
