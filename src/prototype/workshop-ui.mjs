...
export function createWorkshop({ data, getModel, getEmployee, rerender, toast, saved, persist, eventTitle }) {
  ...
  function journey(state) {
    ...
    return `<section class="journey panel" aria-label="Карта развития"><div class="section-head"><div><p class="eyebrow">Квест к вашей цели</p><h2>Карта развития</h2></div><span class="tag">${result ? sourceName(result.source) : 'До 5 последовательных шагов'}</span></div>
      ...
      ${result ? `<p class="workflow-status" role="status">${escape(result.message)}</p>` : '<p class="footnote">План сохраняется в этом браузере. AI сможет выбрать маршрут и объяснить шаги после подключения.</p>'}
      ...
      ${plan && progress.valid ? `<div class="journey-summary"><div><h3>${escape(plan.title)}</h3><p>${escape(plan.summary)}</p><small>${plan.totalHours} ч обучения · ориентир до ${date(plan.end)} · выполнено ${progress.done} из ${plan.steps.length}</small></div><div class="journey-score"><b>${plan.coverageBefore}% → ${plan.coverageAfter}%</b><span>покрытие требований после всех шагов</span></div></div>
        ...
        ${i === progress.done ? `<button class="button secondary" data-action="quest-complete" data-event="${escape(s.event_id)}">Завершить шаг в демо</button>` : ''}</div></li>`).join('')}</ol>
  ...
  function hrPage() {
    ...
    return `<div class="page-heading"><div><h1>Мастерская HR</h1><p class="subtitle">Превратите активность в практику, которая помогает применять навыки в работе.</p></div><span class="tag">Черновик → пилот → проверка</span></div>
      ...
      <p class="footnote">Черновики хранятся только в этом браузере. Для обсуждения с командой скачайте JSON. Обязательные активности не участвуют в мастерской; данные об аудитории агрегированы. Экран HR — демонстрация без разграничения доступа.</p>`;
  }
  ...
  function saveDraft(form) {
    ...
    result.draft = draft;
    const success = save();
    document.querySelector('#draft-save-status').textContent = success ? 'Сохранено в этом браузере' : 'Не удалось сохранить в браузере — скачайте файл';
    return true;
  }
  ...
  return {
    ...
    action(name, button) {
      if (name === 'quest-complete') {
        ...
        if (progress.valid && plan.steps[progress.done]?.event_id === button.dataset.event && getModel().complete(state.employee.employee_id, button.dataset.event)) {
          save(); rerender(); toast('Квест завершён в демо. Следующий шаг открыт.');
        }
        return true;
      }
