...
async function body(req) {
  ...
  let size = 0, chunks = [];
  for await (const chunk of req) { size += chunk.length; if (size > 2 * 1024 * 1024) fail(413, 'BODY_TOO_LARGE', 'Максимальный размер запроса — 2 МБ'); chunks.push(chunk); }
...
export function buildServer({ dbPath = ':memory:', dataset, rules, asOf, employeeId = 'E0066', employeeToken, hrToken } = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (path.startsWith('/api/hr/') || path.startsWith('/api/dataset/')) if (user.role !== 'hr') fail(403, 'HR_REQUIRED', 'Доступно только HR');
      ...
      if (['/api/dataset/validate', '/api/dataset/import'].includes(path) && req.method === 'POST') {
        ...
        if (fingerprint(validation.data, incomingRules) === store.version) return send(res, 200, { imported: false, unchanged: true, counts: validation.counts });
        if (!validation.data.employees.some(e => e.employee_id === employeeId)) fail(422, 'MISSING_ACCOUNT', 'Датасет не содержит настроенного сотрудника');
        if (validation.data.asOf > service.asOf) fail(422, 'FUTURE_DATASET', 'Срез датасета позже модельной даты сервера');
        store.transaction(() => {
          const editedGoal = store.db.prepare('SELECT id,goal FROM employees').all().some(e => e.goal !== (service.originalEmployees.get(e.id).career_goal ? JSON.stringify(service.originalEmployees.get(e.id).career_goal) : null));
          if (store.db.prepare('SELECT count(*) AS n FROM completions').get().n || editedGoal) fail(409, 'IMPORT_WOULD_OVERWRITE', 'Есть сохранённые действия. Импортируйте в новую базу данных');
          const seen = new Set(), repeats = new Set(incomingRules.repeatableEventIds);
          const history = [...validation.data.history].sort((a, b) => a.date.localeCompare(b.date) || a.record_id.localeCompare(b.record_id)).filter(h => {
          ...
          const model = createModel({ ...validation.data, history }, incomingRules);
          store.db.exec('DELETE FROM requests; DELETE FROM employees;');
          const insert = store.db.prepare('INSERT INTO employees VALUES(?,?,?)');
          for (const e of validation.data.employees) insert.run(e.employee_id, JSON.stringify(model.snapshot(e).levels), e.career_goal ? JSON.stringify(e.career_goal) : null);
          store.db.prepare('UPDATE dataset SET payload=?,rules=?,fingerprint=? WHERE id=1').run(JSON.stringify(validation.data), JSON.stringify(incomingRules), fingerprint(validation.data, incomingRules));
        });
        store.data = validation.data; store.rules = incomingRules; store.version = fingerprint(store.data, store.rules);
        service = new CareerService(store, service.asOf);
        return send(res, 200, { imported: true, counts: validation.counts });
      }
